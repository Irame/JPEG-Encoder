...
AVXCALL transpose8_ps(__m512 &row0, __m512 &row1, __m512 &row2, __m512 &row3, __m512 &row4, __m512 &row5, __m512 &row6, __m512 &row7) {
	static const __m512i permute1{ 0x00, 0x01, 0x02, 0x03, 0x10, 0x11, 0x12, 0x13, 0x08, 0x09, 0x0a, 0x0b, 0x18, 0x19, 0x1a, 0x1b };
	static const __m512i permute2{ 0x04, 0x05, 0x06, 0x07, 0x14, 0x15, 0x16, 0x17, 0x0c, 0x0d, 0x0e, 0x0f, 0x1c, 0x1d, 0x1e, 0x1f };
	...
	row0 = _mm512_permutex2var_ps(__tt0, permute1, __tt4);
...
AVXCALL oneDimensionalDCT(__m512* ref)
{
	...
	__m512 temp1[8];
	temp1[0] = _mm512_add_ps(ref[0], ref[7]);
	temp1[1] = _mm512_add_ps(ref[1], ref[6]);
	temp1[2] = _mm512_add_ps(ref[2], ref[5]);
	temp1[3] = _mm512_add_ps(ref[3], ref[4]);
	temp1[4] = _mm512_add_ps(ref[3], ref[4]);
	temp1[5] = _mm512_add_ps(ref[2], ref[5]);
	temp1[6] = _mm512_add_ps(ref[1], ref[6]);
	temp1[7] = _mm512_add_ps(ref[0], ref[7]);

	ref[0] = _mm512_add_ps(temp1[0], temp1[3]);
	ref[1] = _mm512_add_ps(temp1[1], temp1[2]);
	ref[2] = _mm512_sub_ps(temp1[1], temp1[2]);
	ref[3] = _mm512_sub_ps(temp1[0], temp1[3]);
	ref[4] = _mm512_add_ps(temp1[4], temp1[5]);
	ref[5] = _mm512_add_ps(temp1[5], temp1[6]);
	ref[6] = _mm512_add_ps(temp1[6], temp1[7]);
	ref[7] = temp1[7];

	temp1[0] = _mm512_add_ps(ref[0], ref[1]);
	temp1[1] = _mm512_sub_ps(ref[0], ref[1]);
	temp1[2] = _mm512_add_ps(ref[2], ref[3]);
	temp1[3] = ref[3];
	temp1[4] = ref[4];
	temp1[5] = ref[5];
	temp1[6] = ref[6];
	...
	__m512 temp6plus4 = _mm512_sub_ps(temp1[6], temp1[4]);
	__m512 a13avx = _mm512_set1_ps(a1);
	__m512 a2avx = _mm512_set1_ps(a2);
	__m512 a4avx = _mm512_set1_ps(a4);
	__m512 a5avx = _mm512_set1_ps(a5);
	temp1[2] = _mm512_mul_ps(temp1[2], a13avx);
	temp1[4] = _mm512_sub_ps(_mm512_mul_ps(temp1[4], a2avx), _mm512_mul_ps(temp6plus4, a5avx));
	temp1[5] = _mm512_mul_ps(temp1[5], a13avx);;
	temp1[6] = _mm512_sub_ps(_mm512_mul_ps(temp1[6], a4avx), _mm512_mul_ps(temp6plus4, a5avx));

	__m512 temp = temp1[2];
	temp1[2] = _mm512_add_ps(temp1[2], temp1[3]);
	temp1[3] = _mm512_sub_ps(temp1[3], temp);
	temp = temp1[5];
	temp1[5] = _mm512_add_ps(temp1[5], temp1[7]);
	temp1[7] = _mm512_sub_ps(temp1[7], temp);

	ref[0] = temp1[0];
	ref[4] = temp1[1];
	ref[2] = temp1[2];
	ref[6] = temp1[3];
	ref[5] = _mm512_add_ps(temp1[4], temp1[7]);
	ref[1] = _mm512_add_ps(temp1[5], temp1[6]);
	ref[7] = _mm512_sub_ps(temp1[5], temp1[6]);
	ref[3] = _mm512_sub_ps(temp1[7], temp1[4]);

	ref[0] = _mm512_mul_ps(ref[0], _mm512_set1_ps(s0));
	ref[1] = _mm512_mul_ps(ref[1], _mm512_set1_ps(s1));
	ref[2] = _mm512_mul_ps(ref[2], _mm512_set1_ps(s2));
	ref[3] = _mm512_mul_ps(ref[3], _mm512_set1_ps(s3));
	ref[4] = _mm512_mul_ps(ref[4], _mm512_set1_ps(s4));
	ref[5] = _mm512_mul_ps(ref[5], _mm512_set1_ps(s5));
	ref[6] = _mm512_mul_ps(ref[6], _mm512_set1_ps(s6));
	ref[7] = _mm512_mul_ps(ref[7], _mm512_set1_ps(s7));
}

AVXCALL quantifyAndRoundDCTAVX(__m512* ref, const __m256* qTable)
{
    static const int roundingParameter = (_MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
	for (size_t i = 0; i < 8; i++)
	{
		ref[i] = _mm512_div_round_ps(ref[i], _mm512_maskz_broadcast_f32x8(0xffff, qTable[i]), roundingParameter);
	}
}

AVXCALL twoDimensionalDCTAVX(__m512* regs)
{
	...
	const __m512 dcCorrectionValue = _mm512_set_ps(0, 0, 0, 0, 0, 0, 0, 1024, 0, 0, 0, 0, 0, 0, 0, 1024);
	regs[0] = _mm512_sub_ps(regs[0], dcCorrectionValue);
}


AVXCALL twoDimensionalDCTAVX(const PointerMatrix& in1, const PointerMatrix& in2, PointerMatrix& out1, PointerMatrix& out2)
{
	__m512 regs[8]
	{
		_mm512_mask_loadu_ps(_mm512_maskz_loadu_ps(0x00ff, in1[0]), 0xff00, in2[0]),
		_mm512_mask_loadu_ps(_mm512_maskz_loadu_ps(0x00ff, in1[1]), 0xff00, in2[1]),
		_mm512_mask_loadu_ps(_mm512_maskz_loadu_ps(0x00ff, in1[2]), 0xff00, in2[2]),
		_mm512_mask_loadu_ps(_mm512_maskz_loadu_ps(0x00ff, in1[3]), 0xff00, in2[3]),
		_mm512_mask_loadu_ps(_mm512_maskz_loadu_ps(0x00ff, in1[4]), 0xff00, in2[4]),
		_mm512_mask_loadu_ps(_mm512_maskz_loadu_ps(0x00ff, in1[5]), 0xff00, in2[5]),
		_mm512_mask_loadu_ps(_mm512_maskz_loadu_ps(0x00ff, in1[6]), 0xff00, in2[6]),
		_mm512_mask_loadu_ps(_mm512_maskz_loadu_ps(0x00ff, in1[7]), 0xff00, in2[7]),
	};

	twoDimensionalDCTAVX(regs);

	_mm512_mask_compressstoreu_ps(out1[0], 0x00ff, regs[0]);
	_mm512_mask_compressstoreu_ps(out1[1], 0x00ff, regs[1]);
	_mm512_mask_compressstoreu_ps(out1[2], 0x00ff, regs[2]);
	_mm512_mask_compressstoreu_ps(out1[3], 0x00ff, regs[3]);
	_mm512_mask_compressstoreu_ps(out1[4], 0x00ff, regs[4]);
	_mm512_mask_compressstoreu_ps(out1[5], 0x00ff, regs[5]);
	_mm512_mask_compressstoreu_ps(out1[6], 0x00ff, regs[6]);
	_mm512_mask_compressstoreu_ps(out1[7], 0x00ff, regs[7]);

	_mm512_mask_compressstoreu_ps(out2[0], 0xff00, regs[0]);
	_mm512_mask_compressstoreu_ps(out2[1], 0xff00, regs[1]);
	_mm512_mask_compressstoreu_ps(out2[2], 0xff00, regs[2]);
	_mm512_mask_compressstoreu_ps(out2[3], 0xff00, regs[3]);
	_mm512_mask_compressstoreu_ps(out2[4], 0xff00, regs[4]);
	_mm512_mask_compressstoreu_ps(out2[5], 0xff00, regs[5]);
	_mm512_mask_compressstoreu_ps(out2[6], 0xff00, regs[6]);
	_mm512_mask_compressstoreu_ps(out2[7], 0xff00, regs[7]);
}

AVXCALL twoDimensionalDCTandQuantisationAVX(const PointerMatrix& in1, const PointerMatrix& in2, const QTable& qTable, PointerMatrix& out1, PointerMatrix& out2)
{
    __m512 regs[8]
	{
        _mm512_mask_loadu_ps(_mm512_maskz_loadu_ps(0x00ff, in1[0]), 0xff00, in2[0]),
	...
	const __m512 factor256 = _mm512_set1_ps(255);
	//for (size_t i = 0; i < 8; i++)
	//{
		regs[0] = _mm512_mul_ps(regs[0], factor256);
		regs[1] = _mm512_mul_ps(regs[1], factor256);
		regs[2] = _mm512_mul_ps(regs[2], factor256);
		regs[3] = _mm512_mul_ps(regs[3], factor256);
		regs[4] = _mm512_mul_ps(regs[4], factor256);
		regs[5] = _mm512_mul_ps(regs[5], factor256);
		regs[6] = _mm512_mul_ps(regs[6], factor256);
		regs[7] = _mm512_mul_ps(regs[7], factor256);
	//}

	twoDimensionalDCTAVX(regs);

	quantifyAndRoundDCTAVX(regs, qTable.avx);
