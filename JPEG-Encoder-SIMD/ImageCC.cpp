...
ImageCC::ImageCC(size_t width, size_t height, SamplingScheme scheme) 
...
void ImageCC::setRawPixelDataDirect(float* rgbaData)
{
	int pixelCount = width*height;
	transposeFloatAVX(rgbaData, channels->red(), channels->green(), channels->blue(), pixelCount);
}

void ImageCC::setRawPixelData(float* rgbaData)
{
...
std::vector<float> ImageCC::getRawPixelDataSimulated()
{
...
std::vector<float> ImageCC::getRawPixelData()
{
	if (stepX == 1 && stepY == 1)
	{
...
void ImageCC::SetPixel(uint x, uint y, const PixelData32& color)
{
...
void ImageCC::GetPixel(PixelData32& ref, uint x, uint y) const
{
	size_t pos = simulatedWidth * y + x;
...
void ImageCC::convertToYCbCrAVX()
{
...
void ImageCC::multiplyColorChannelByAVX(int colorChannel, float val)
{
	multiplyAVX(channels->getChannel(colorChannel), val, blocksPerChannel[colorChannel] * 8);
}

size_t ImageCC::getWidth() const
{
	return width;
}

size_t ImageCC::getHeight() const
{
	return height;
}

size_t ImageCC::getSimulatedWidth() const
{
	return simulatedWidth;
}

size_t ImageCC::getSimulatedHeight() const
{
